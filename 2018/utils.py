...
def dijkstra(
    ...
    # (f, g, n)
    todo = [
        (0 + heuristic(from_node), 0, from_node)]  # type: typing.List[typing.Tuple[int, int, T]]
...
def hamming_distance(a, b) -> int:
    return sum(
        i is BLANK or j is BLANK or i != j for i, j in itertools.zip_longest(a, b, fillvalue=BLANK))
...
def get_actual(day=None, year=None):
    ...
    opener.addheaders = [("Cookie", "session={}".format(token)),
